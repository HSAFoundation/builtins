...
float hsa_fmaf(float a, float b, float c)
{
    if (HAVE_HW_FMA32())
    {
		#if !defined(SUBNORMALS_SUPPORTED)
			if(a*a == 0.  || b*b == 0.0 || c+c ==0.)
			{
				uint x1 = as_uint(a);
				uint x2 = as_uint(b);
				uint x3 = as_uint(c);

				double d1 = float_uint_to_double(x1);
				double d2 = float_uint_to_double(x2);
				double d3 = float_uint_to_double(x3);
				d1 = fma(d1,d2,d3);
				x1 = double_to_float_uint(d1);
